...
output <- foreach(i = 1109:lenDF,.packages = 'data.table') %dopar% {
  starttime <- Sys.time()
  j <- keywords[i,]
  print(lenDF - i)
  x <- queryRankings(j[,2], j[,4] , startDate, endDate, limit)
  tmp <- rbindlist(list(x, tmp), fill = TRUE)
...
output <- foreach(i = 1109:1013,.packages = 'data.table') %do% {
  starttime <- Sys.time()
  j <- keywords[i,]
  print(lenDF - i)
  x <- queryRankings(j[,2], j[,4] , startDate, endDate, limit)
