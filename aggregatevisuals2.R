...
three  <- dt %>% 
  ...
  dplyr::select(monSlice, Keyword, Host, Domain, rsum, pmean, isum, Product, NumberKeywords)  %>% 
  dplyr::group_by(monSlice, Host, Domain, Product) %>% 
...
Out_three<- three  %>% 
  dplyr::ungroup() %>%
  dplyr::select(Host, Domain, rsum, pmean, isum, Product, n) %>% 
